    ...
    def _save_graphs(self, out: Path, metrics: List[Any], timing: Dict[str, Any], contexts: List[Any]) -> Dict[str, Path]:
        ...
        if self.include_spans:
            ...
            for c in contexts or []:
                ...
                for k, v in sd.items():
                    spans.setdefault(str(k), []).append(float(v))
            if spans:
        ...
        if metrics:
            ...
            if self.include_detector_breakdown:
                ...
                if mm:
                    ...
                    if per_class:
                        items = sorted(((getattr(pc, "label", "-"), float(_num(getattr(pc, "ap", 0.0))), int(_num(getattr(pc, "support", 0)))) for pc in per_class), key=lambda x: x[1])
                        # Inline casting without _num helper
                        items = sorted(((
                            getattr(pc, "label", "-"),
            ...
            "metrics": [
                {
                    ...
                    "score": float(getattr(m, "score", 0.0)),
                    # Inline dataclass/dict handling without _asdict helper
