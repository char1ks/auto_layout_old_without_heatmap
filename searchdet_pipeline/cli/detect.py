...
def execute_detect (args )->int :
    ...
    try :
        ...
        if use_fastsam_optimized :
            print ("6️⃣ Вызов: detector.find_present_elements_with_fastsam_optimized() [ОПТИМИЗИРОВАННЫЙ FASTSAM РЕЖИМ]")
            print ("   ↳ Это запустит оптимизированный FastSAM пайплайн:")
            print ("   ↳ _load_example_images() → FastSAM (адаптивный режим) → _score_masks() → результат")
            result =detector .find_present_elements_with_fastsam_optimized (
            args .image ,
            args .positive ,
            args .negative ,
            args .output if hasattr (args ,'output')and args .output else "output"
            )
        elif use_fastsam_with_heatmap :
            print ("6️⃣ Вызов: detector.find_present_elements_with_fastsam_integration() [FASTSAM+HEATMAP РЕЖИМ]")
            ...
            result =detector .find_present_elements_with_fastsam_integration (
            args .image ,
            args .positive ,
            args .negative ,
            args .output if hasattr (args ,'output')and args .output else "output"
            )
        elif use_heatmap or use_heatmap_sam_hybrid :
            mode_desc ="ГИБРИДНЫЙ HEATMAP+SAM РЕЖИМ"if use_heatmap_sam_hybrid else "HEATMAP РЕЖИМ"
            print (f"6️⃣ Вызов: detector.find_present_elements_with_heatmap() [{mode_desc}]")
            if use_heatmap_sam_hybrid :
                print ("   ↳ Это запустит гибридный пайплайн:")
                print ("   ↳ _load_example_images() → HeatmapGenerator → _extract_hotspots_from_heatmap() → _refine_hotspots_with_sam() → BinningProcessor")
            else :
