...
class EmbeddingExtractor :
    ...
    def _encode_mask_roi_with_dinov3 (self ,image_np :np .ndarray ,mask_bool :np .ndarray ,pad :int =8 )->np .ndarray :
        ...
        if m_crop .any ():
            ...
            if crop.shape[:2] == m_crop.shape:
                mean_color = crop [m_crop ].mean (axis =0 )
            else:
                # Если размерности не совпадают, используем весь crop для вычисления среднего
                mean_color = crop.reshape(-1, crop.shape[-1]).mean(axis=0)
        else:
            mean_color = np .array ([128 ,128 ,128 ],dtype =np .float32 )
        bg =np .tile (mean_color .reshape (1 ,1 ,3 ),(crop .shape [0 ],crop .shape [1 ],1 ))
        # Исправляем broadcasting - убеждаемся, что размерности совпадают
        if crop.shape[:2] == m_crop.shape:
            crop =np .where (m_crop [...,None ],crop ,bg ).astype (np .uint8 )
        else:
            # Если размерности не совпадают, используем альтернативный подход
            crop_resized = crop[:m_crop.shape[0], :m_crop.shape[1]]
            bg_resized = bg[:m_crop.shape[0], :m_crop.shape[1]]
            crop = np.where(m_crop[..., None], crop_resized, bg_resized).astype(np.uint8)
    ...
    def _encode_masks_batch_optimized (self ,image_np :np .ndarray ,masks :list ,batch_size :int =64 )->np .ndarray :
        ...
        for i in range (0 ,len (masks ),batch_size ):
            ...
            for mask_data in batch_masks :
                if isinstance (mask_data ,dict )and 'segmentation'in mask_data :
                    ...
                    # ОПТИМИЗАЦИЯ: Упрощенное вычисление среднего цвета
                    if m_crop .any () and crop.shape[:2] == m_crop.shape:
                        mean_color =crop [m_crop ].mean (axis =0 )
                    else :
                        mean_color =crop.reshape(-1, crop.shape[-1]).mean(axis=0) if crop.size > 0 else np .array ([128 ,128 ,128 ],dtype =np .float32 )

                    # ОПТИМИЗАЦИЯ: Упрощенное применение маски
                    if crop.shape[:2] == m_crop.shape:
    ...
    def extract_mask_embeddings (self ,image_pil ,masks ):
        ...
        if sam_masks :
            try :
                batch_embeddings =self ._encode_masks_batch_optimized (image_np ,sam_masks ,batch_size =64 )
    ...
    def extract_embeddings_from_masks (self ,image_np :np .ndarray ,masks :list )->np .ndarray :
        """
        Извлекает эмбеддинги из масок для изображения.
        
        Args:
            image_np: Изображение в формате numpy array (H, W, 3)
            masks: Список масок в формате SAM/FastSAM
            
        Returns:
            np.ndarray: Массив эмбеддингов размером (N, D)
        """
        # Конвертируем numpy array в PIL Image для совместимости
        if isinstance(image_np, np.ndarray):
            image_pil = Image.fromarray(image_np.astype(np.uint8))
        else:
            image_pil = image_np
            
        return self.extract_mask_embeddings(image_pil, masks)
