...
class FastSAMHeatmapProcessor :
    ...
    def process_image (self ,image :Image .Image ,pos_by_class :Dict [str ,np .ndarray ],heatmap :Optional [torch .Tensor ]=None ,neg_imgs :np .ndarray =None ,heatmap_threshold :float =0.5 ,crop_padding :int =20 ,min_overlap_ratio :float =0.7 ,skip_scoring_for_hotspot_masks :bool =False )->List [torch .Tensor ]:
        ...
        try :
            ...
            # Извлекаем hotspots из heatmap для более точной генерации масок
            if hasattr(self, 'heatmap_generator') and self.heatmap_generator is not None:
                print ("🎯 Извлечение hotspots из heatmap...")
                cropped_heatmap_np = heatmap.cpu().numpy() if isinstance(heatmap, torch.Tensor) else heatmap
                hotspots = self._extract_hotspots_from_heatmap(cropped_heatmap_np, threshold=0.7, max_hotspots=8)
                
                if hotspots:
                    print (f"🔥 Найдено {len(hotspots)} hotspots, генерируем FastSAM маски с их использованием")
                    fastsam_masks = self._generate_fastsam_masks_with_hotspots(cropped_image, hotspots)
                else:
                    print ("⚠️ Hotspots не найдены, используем стандартную генерацию FastSAM")
                    fastsam_masks = self._generate_fastsam_masks(cropped_image)
            else:
                fastsam_masks = self._generate_fastsam_masks(cropped_image)
    ...
    def _generate_fastsam_masks_fast (self ,cropped_image :Image .Image )->List [torch .Tensor ]:
        ...
        try :
            ...
            # ОПТИМИЗАЦИЯ: Уменьшенные размеры для ускорения
            max_size =384 if self .fast_mode else 512  # Уменьшено
            if max (cropped_image .size )>max_size :
                ...
                cropped_image =cropped_image .resize (new_size ,Image .Resampling .NEAREST )  # Более быстрая интерполяция

            image_np =np .array (cropped_image )

            # ОПТИМИЗАЦИЯ: Упрощенные параметры
            results =self .fastsam_model (image_np ,device ='cuda'if torch .cuda .is_available ()else 'cpu',retina_masks =False ,imgsz =256 ,conf =0.7 ,iou =0.8 ,verbose =False)

            masks =[]
            if len (results )>0 and hasattr (results [0 ],'masks')and results [0 ].masks is not None :
                mask_data =results [0 ].masks .data

                # ОПТИМИЗАЦИЯ: Ограничиваем количество масок
                num_masks =min (len (mask_data ),8 )  # Уменьшено
    ...
    def _generate_fastsam_masks_with_hotspots (self ,image :Image .Image ,hotspots :List [Tuple [int ,int ]])->List [torch .Tensor ]:
        ...
        try :
            ...
            # ОПТИМИЗАЦИЯ: Уменьшенный размер
            target_long =512  # Уменьшено с 1024
            ...
            try:
                ...
                prompted_masks = results[0].masks.data if results[0].masks is not None else []
                if len(prompted_masks) == 0:
                    return self._generate_fallback_masks(image)
    ...
    skip_scoring :bool =False )->Tuple [List [Dict [str ,Any ]],List [torch .Tensor ]]:
        ...
        try :
            ...
            decisions ,debug_info =self .score_calculator .score_multiclass (mask_vecs ,q_pos_ready ,q_neg_ready)
...
class FastSAMProcessor :
    ...
    def __init__ (self ,fastsam_model =None ,device ='auto', heatmap_generator=None):
    ...
    def _filter_masks_by_heatmap_overlap (self ,masks :List [torch .Tensor ],heatmap :torch .Tensor ,overlap_threshold :float =0.7 )->List [torch .Tensor ]:
        ...
        try :
            ...
            if len (masks )>0 :
                ...
                # ОПТИМИЗАЦИЯ: Упрощенная интерполяция
                if heatmap .shape !=(mask_h ,mask_w ):
                    heatmap_resized =torch .nn .functional .interpolate (heatmap .unsqueeze (0 ).unsqueeze (0 ).float (),size =(mask_h ,mask_w ),mode ='nearest').squeeze ()  # Изменено на nearest
                else :
                    heatmap_resized =heatmap
                hot_zones =(heatmap_resized >0.4 ).float ()  # Повышен порог с 0.3 до 0.4
                filtered_masks =[]
                rejected_masks =[]
                # ОПТИМИЗАЦИЯ: Ограничиваем обработку первыми 10 масками
                for i ,mask in enumerate (masks [:10]):
                    ...
                    if mask_area >0 :
                        ...
                        # ОПТИМИЗАЦИЯ: Убираем печать для ускорения
                        if overlap_ratio >=overlap_threshold :
                            filtered_masks .append (mask )
                        else :
                            rejected_masks .append ((i ,overlap_ratio ))
                # ОПТИМИЗАЦИЯ: Убираем избыточную печать
                return filtered_masks
        except Exception as e :
