...
class MaskFilter :
    ...
    def apply_all_filters (self ,masks :List [Dict [str ,Any ]],image_np :np .ndarray )->List [Dict [str ,Any ]]:
        ...
        # ОПТИМИЗАЦИЯ: Пропускаем сложные фильтры при малом количестве масок
        if self .smart_rectangle_filter and len(masks) > 10:
            initial_count =len (masks )
            masks =self ._filter_perfect_rectangles_optimized (masks[:15])  # Ограничиваем обработку
            dropped =initial_count -len (masks )
            if dropped >0 :
                print (f"🔳 Удалено {dropped} прямоугольных масок")

        if self .border_ban:  # Изменено условие
            initial_count =len (masks )
        ...
        if len(masks) > 8:  # ОПТИМИЗАЦИЯ: Ограничиваем обработку
            initial_count =len (masks )
            masks =self ._filter_nested_masks_optimized (masks[:12])  # Ограничиваем обработку
            dropped =initial_count -len (masks )
            if dropped >0 :
                print (f"🔗 Удалено {dropped} вложенных масок")

        # ОПТИМИЗАЦИЯ: Убираем исправление масок для ускорения
