...
class HeatmapGenerator :
    ...
    sharpening_factor :float =1.5 )->torch .Tensor :
        ...
        enhanced =torch .pow (similarities_norm ,gamma )

        enhanced =torch .sigmoid (sharpening_factor *(enhanced -0.5 ))

        enhanced =enhanced *(similarities .max ()-similarities .min ())+similarities .min ()
    ...
    kernel_size :int =3 )->torch .Tensor :
        ...
        kernel_size =getattr (self ,'morph_smoothing_kernel',5 )

        if kernel_size %2 ==0 :
            kernel_size +=1

        sigma =kernel_size /3.0
        x =torch .arange (kernel_size ,device =heatmap .device ,dtype =torch .float32 )-kernel_size //2
        ...
        smoothed =F .conv2d (heatmap_expanded ,gaussian_2d ,padding =kernel_size //2 )
