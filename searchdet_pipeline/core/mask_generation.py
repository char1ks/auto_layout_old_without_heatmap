...
class MaskGenerator :
    ...
    def __init__ (self ,mask_backend :str ,device :str ='cpu',fastsam_model :str =None ,
    fastsam_device :str =None ,sam_generator =None ,**params )->None :
        self .mask_backend =mask_backend
        self .device =device
        self .fastsam_model =fastsam_model
        self .fastsam_device =fastsam_device or device
        self .sam_generator =sam_generator
        self .params =params
        self ._fastsam_model :Optional [Any ]=None
        self ._sam_generator :Optional [Any ]=None
        self ._preload_models ()

    def _preload_models (self ):

        print ("🔄 Предзагрузка SAM модели...")
        if self .mask_backend =="fastsam":
            self ._preload_fastsam ()
        elif self .mask_backend =="sam-hq":
            self ._preload_sam_hq ()
        elif self .mask_backend =="sam":

            if self .sam_generator is None :
                print ("   ⚠️ SAM-генератор не был предоставлен для 'sam' бэкенда, ожидается внешняя инициализация.")
        else :
            print (f"⚠️ Неизвестный бэкенд: {self.mask_backend}, модели не предзагружены.")

    def _preload_fastsam (self )->None :

        print ("🚀 Инициализация FastSAM...")
        self ._fastsam_model =_load_fastsam_model (self .fastsam_model )
        print ("✅ FastSAM готов")

    def _preload_sam_hq (self )->None :
    ...
    def generate (self ,image_np :np .ndarray )->List [Dict [str ,Any ]]:
        ...
        print (f"🚀 ЭТАП 1: {self.mask_backend.upper()} автогенерация масок")

        if self .mask_backend =="fastsam":
            return self ._generate_fastsam_masks (image_np )
        elif self .mask_backend in ["sam-hq","sam"]:
            return self ._generate_sam_masks (image_np )
        else :
            raise ValueError (f"Неподдерживаемый бэкенд: {self.mask_backend}")

    def _generate_fastsam_masks (self ,image_np :np .ndarray )->List [Dict [str ,Any ]]:
        ...
        max_size =self .params .get ('fastsam_max_size',512 )
        scale =1.0
        if max (H0 ,W0 )>max_size :
            scale =max_size /max (H0 ,W0 )
            new_h ,new_w =int (H0 *scale ),int (W0 *scale )
            run_img =cv2 .resize (image_np ,(new_w ,new_h ),interpolation =cv2 .INTER_LINEAR )
        else :
            run_img =image_np

        h ,w =run_img .shape [:2 ]

        fastsam_imgsz =self .params .get ('fastsam_imgsz',320 )

        if fastsam_imgsz =='None'or fastsam_imgsz is None :
            fastsam_imgsz =320
        elif isinstance (fastsam_imgsz ,str ):
            if fastsam_imgsz .lower ()in ['none','null','']:
                fastsam_imgsz =320
            else :
                try :
                    fastsam_imgsz =int (fastsam_imgsz )
                except ValueError :
                    fastsam_imgsz =320

        imgsz =min (fastsam_imgsz ,320 )
        conf =self .params .get ('fastsam_conf',0.6 )
        iou =self .params .get ('fastsam_iou',0.7 )
        retina =False

        print (f"   ⚡ Быстрый режим FastSAM")
        ...
        max_masks =self .params .get ('max_fastsam_masks',20 )
        masks_np =masks_np [:max_masks ]

        out =[]
        min_area =self .params .get ('min_mask_area',500 )

        for seg in masks_np :
            seg_u8 =(seg >0 ).astype (np .uint8 )

            area_before =np .sum (seg_u8 )
            if area_before <min_area //(scale *scale ):
                continue
    ...
    def _generate_sam_masks (self ,image_np ):
        ...
        run_img =image_np
        scale =1.0
        sam_long_side =self .params .get ('sam_long_side',None )

        if sam_long_side =='None'or sam_long_side is None :
            sam_long_side =None
        elif isinstance (sam_long_side ,str ):
            if sam_long_side .lower ()in ['none','null','']:
                sam_long_side =None
            else :
                try :
                    sam_long_side =int (sam_long_side )
                except ValueError :
                    sam_long_side =None
    ...
    def _init_sam_hq (self ):
        ...
        sam_checkpoint =self .params .get ('sam_model',None )
        ...
        model_type =self .params .get ('sam_encoder','vit_l')or 'vit_l'
    ...
    def _find_sam_checkpoint (self ):
        ...
        model_type =self .params .get ('sam_encoder','vit_l')
        ...
        if 'sam_encoder'not in self .params :
