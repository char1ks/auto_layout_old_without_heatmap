...
class MaskGenerator :
    ...
    def _generate_fastsam_masks (self ,image_np :np .ndarray )->List [Dict [str ,Any ]]:
        ...
        max_size =self .params .get ('fastsam_max_size',384 )
        scale =1.0
        if max (H0 ,W0 )>max_size :
            scale =max_size /max (H0 ,W0 )
            new_h ,new_w =int (H0 *scale ),int (W0 *scale )
            run_img =cv2 .resize (image_np ,(new_w ,new_h ),interpolation =cv2 .INTER_AREA )
        else :
            run_img =image_np
        ...
        imgsz =min (fastsam_imgsz ,256 )
        conf =self .params .get ('fastsam_conf',0.7 )
        iou =self .params .get ('fastsam_iou',0.8 )
        retina =False
        ...
        max_masks =self .params .get ('max_fastsam_masks',15 )
