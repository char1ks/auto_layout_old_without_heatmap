...
class MaskGenerator:
    ...
    def _preload_models(self) -> None:
        ...
        print(f"🔄 Предзагрузка {self.mask_backend.upper()} модели...")
        ...
        if self.mask_backend == "fastsam":
            ...
            print(f"⚠️ Неизвестный бэкенд: {self.mask_backend}")
    ...
    def _preload_fastsam(self) -> None:
        ...
        self._fastsam_device = self.fastsam_device or ('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    def generate(self, image_np: np.ndarray) -> List[Dict[str, Any]]:
        ...
        Args:
            image_np: Изображение в формате numpy array (RGB)
            
        Returns:
            Список словарей с масками и метаданными
        """
    ...
    def _generate_fastsam_masks(self, image_np: np.ndarray) -> List[Dict[str, Any]]:
        """Генерирует маски с помощью FastSAM.
        
        Args:
            image_np: Изображение в формате numpy array (RGB)
            
        Returns:
            Список словарей с масками FastSAM
        """
        ...
        imgsz = self.params.get('fastsam_imgsz', 1024)
        conf = self.params.get('fastsam_conf', 0.4)
        iou = self.params.get('fastsam_iou', 0.9)
        retina = self.params.get('fastsam_retina', True)
