"""Датаклассы для структурирования данных в searchdet_pipeline."""
...
class MaskBackend(Enum):
    """Поддерживаемые бэкенды для генерации масок."""
    FASTSAM = "fastsam"
    SAM = "sam"
    SAM2 = "sam2"
    SAM_HQ = "sam_hq"
...
class BackboneType(Enum):
    """Поддерживаемые типы backbone моделей."""
    RESNET101 = "resnet101"
    DINOV2_S = "dinov2_s"
    DINOV2_B = "dinov2_b"
    DINOV2_L = "dinov2_l"
...
@dataclass
class BoundingBox:
    """Представление ограничивающего прямоугольника."""
    x: int
    y: int
    width: int
    height: int
...
@dataclass
class MaskData:
    """Данные маски с метаинформацией."""
...
@dataclass
class EmbeddingData:
    """Данные эмбеддинга с метаинформацией."""
    embedding: np.ndarray
    mask_id: int
    source_type: str  # 'positive', 'negative', 'target'
...
class DetectorConfig:
    ...
    max_masks: int = 1000
    min_area_fraction: float = 0.001  # Минимальная доля площади от изображения
    max_area_fraction: float = 0.9    # Максимальная доля площади от изображения
    containment_iou: float = 0.8      # IoU для фильтрации вложенных масок
    ...
    topk: int = 5
    positive_aggregation: str = "mean"  # "mean", "max", "median"
    
    # Параметры DINOv3
    dinov3_backbone: Optional[str] = None
    vit_pooling: str = "cls"  # "cls", "mean", "max"
...
@dataclass
class TimingInfo:
    """Информация о времени выполнения различных этапов."""
