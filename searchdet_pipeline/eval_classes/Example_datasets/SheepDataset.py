...
class SheepDataset(Dataset):
    @classmethod
    def from_json(cls, obj: dict[str, Any]) -> "SheepDataset":
        annotations = cls._build_annotations(obj, base_dir=None)
        meta = obj.get('meta', {})
        meta.update({
            'dataset_type': 'sheep_detection',
    ...
    @classmethod
    def from_path(cls, path: Path) -> "SheepDataset":
        import json
    ...
    def _build_annotations(cls, obj: dict[str, Any], base_dir: Path | None) -> List[COCOAnnotation]:
        ...
        for ann in obj.get('annotations', []) or []:
            try:
                ...
                label: int | str = (
                    cat.get('name') if isinstance(cat, dict) and 'name' in cat 
                ...
                print(f"Пропущена аннотация из-за ошибки: {e}")
                continue
